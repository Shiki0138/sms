...
import { Customer, Reservation, ServiceHistory, Message, Staff } from '../types';
...
// 🎨 顧客に応じたメニュー選択
const getRandomMenu = (customer: Customer): string => {
...
// 💰 メニュー価格設定
const getMenuPrice = (customer: Customer): number => {
  const basePrice = {
    'カット': 4000,
    'カット＋カラー': 8000,
    'カット＋パーマ': 10000,
...
// 📱 予約経路
const getBookingSource = (customer: Customer): string => {
  if (customer.lineId) return 'LINE';
  if (customer.instagramId) return 'Instagram';
  if (customer.tags?.includes('VIP')) return '電話';
  return 'Web';
};

// 📊 施術履歴データ
export const demoServiceHistory: ServiceHistory[] = demoCustomers.flatMap(customer => {
  const historyCount = Math.floor(Math.random() * 3) + 3; // 3-5件の履歴
  const histories: ServiceHistory[] = [];
