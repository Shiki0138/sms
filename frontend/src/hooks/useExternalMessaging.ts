...
export interface ExternalMessagingResult {
  success: boolean
  messageId?: string
  error?: string
...
export const useExternalMessaging = () => {
  ...
  const attemptSendMessage = useCallback(async (
    messageData: ExternalMessageData
  ): Promise<ExternalMessagingResult> => {
    // 環境チェック
   ...
   * 実際の送信処理（本番環境でのみ実行）
   */
  const performActualSend = async (messageData: ExternalMessageData): Promise<ExternalMessagingResult> => {
   ...
   * LINE メッセージ送信
   */
  const sendLineMessage = async (payload: any): Promise<ExternalMessagingResult> => {
   ...
   * Instagram DM送信
   */
  const sendInstagramDM = async (payload: any): Promise<ExternalMessagingResult> => {
   ...
   * SMS送信
   */
  const sendSMS = async (payload: any): Promise<ExternalMessagingResult> => {
   ...
   * メール送信
   */
  const sendEmail = async (payload: any): Promise<ExternalMessagingResult> => {
   ...
   * 特定の機能が利用可能かチェック
   */
  const checkFeatureAvailability = useCallback((feature: string): boolean => {
    switch (feature) {
      case 'line':
   ...
   * 警告表示を試行
   */
  const showRestrictionWarning = useCallback((feature: string) => {
