 ...
 * 環境制限チェックミドルウェア
 */
export const environmentMiddleware = async (request: APIRequest): Promise<APIResponse> => {
  const config = getEnvironmentConfig()
  
  // 制限対象エンドポイントかチェック
  const restriction = Object.entries(RESTRICTED_ENDPOINTS).find(([endpoint]) => 
 ...
 * 実際のAPI呼び出しを実行
 */
const executeAPICall = async (request: APIRequest): Promise<APIResponse> => {
  const config = getEnvironmentConfig()
 ...
 * 制限メッセージを取得
 */
const getRestrictionMessage = (warningType: string): string => {
